...
void send_Timer1OnKeyboard() {
  String kb = "[[\"+1 MIN (T1 ON)\",\"+10 MIN (T1 ON)\"],[\"+1 HOUR (T1 ON)\",\"TURN OFF (T1 ON)\"],[\"BACK\"]]";
  bot.sendMessageWithReplyKeyboard(chat_id, "Timer panel", "", kb, true, false, false);
}

void send_Timer2OnKeyboard() {
  String kb = "[[\"+1 MIN (T2 ON)\",\"+10 MIN (T2 ON)\"],[\"+1 HOUR (T2 ON)\",\"TURN OFF (T2 ON)\"],[\"BACK\"]]";
  bot.sendMessageWithReplyKeyboard(chat_id, "Timer panel", "", kb, true, false, false);
}
...
void check_telegram(){
  ...
  for (uint8_t i = 0; i < num; i++) {
    ...
    // ====== TIMER 1 ON ======
    if (telegram_data == "+1 MIN (T1 ON)") addTimer(timer1_on_active, timer1_on, timer1_on_end, 60UL*1000UL, disableRelay1);
    if (telegram_data == "+10 MIN (T1 ON)") addTimer(timer1_on_active, timer1_on, timer1_on_end, 10UL*60UL*1000UL, disableRelay1);
    if (telegram_data == "+1 HOUR (T1 ON)") addTimer(timer1_on_active, timer1_on, timer1_on_end, 60UL*60UL*1000UL, disableRelay1);
    if (telegram_data == "TURN OFF (T1 ON)") stopTimer(timer1_on_active, timer1_on, "Timer 1 ON stopped");

    // ====== TIMER 2 ON ======
    if (telegram_data == "+1 MIN (T2 ON)") addTimer(timer2_on_active, timer2_on, timer2_on_end, 60UL*1000UL, disableRelay2);
    if (telegram_data == "+10 MIN (T2 ON)") addTimer(timer2_on_active, timer2_on, timer2_on_end, 10UL*60UL*1000UL, disableRelay2);
    if (telegram_data == "+1 HOUR (T2 ON)") addTimer(timer2_on_active, timer2_on, timer2_on_end, 60UL*60UL*1000UL, disableRelay2);
...
void statusRelays() {
  String msg = "Relay 1 " +String(digitalRead(RELAY_1) == LOW ? "ON" : "OFF") +" | Timer OFF in " +formatTime(timer1_off_end) +" | Timer ON in "+formatTime(timer1_on_end);
  msg     += "\nRelay 2 " +String(digitalRead(RELAY_2) == LOW ? "ON" : "OFF") +" | Timer OFF in " +formatTime(timer2_off_end) +" | Timer ON in "+formatTime(timer2_on_end);
  bot.sendMessage(chat_id, msg, "");
}
...
void loop() {
  if (flag_i) {
    ...
    if (count >= 5){
      ...
      checkTimer(timer1_on_active, timer1_on, timer1_on_end, activateRelay1, "Timer 1 ON finished");
      checkTimer(timer2_on_active, timer2_on, timer2_on_end, activateRelay2, "Timer 2 ON finished");
    }
  }
}